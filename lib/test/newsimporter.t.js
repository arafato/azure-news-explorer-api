...
describe("News Importer", function () {
  ...
  describe("Full integration test", function() {
    it("it will export every Azure announcements to DB", function(done) {
      this.timeout(900000);
      newsImporter.getLowerBoundDate(function (err, date) {
        if (err) {
            console.log("ERROR in getLowerBound: " + JSON.stringify(err));
            console.done(err);
        }
        newsImporter.getMostRecentNews(date, function (data) {
          console.log("GOT announcements: " + data.length);
            newsImporter.updateNewsDB(data, function (err, data) {
                if (err) {
                    console.log("ERROR in updateNewsDB: " + JSON.stringify(err));
                    done(err);
                }
