...
function buildKeywordQueryString(keyword, ordermode, startdate, enddate) {
...
module.exports = {
	...
	*/
	keywordQuery: function (keyword, ordermode, startdate, enddate, cb) {
		ordermode = ordermode || 3;
		startdate = startdate || 1199224800; // Azure Birthday
		enddate = enddate || (Date.now() / 1000 | 0);

		var q = buildKeywordQueryString(keyword, ordermode, startdate, enddate);
		request(buildHeader(q), function (err, response, body) {
			if (err || response.statusCode !== 200) {
				cb('Error in retrieving ressources: ' + JSON.stringify(err));
			}
